...
(function exposePhase5Replay() {
  const commonJS = typeof module !== "undefined" && module.exports;
  const configApi = commonJS ? require("./config.js") : window.AeonPhase5Config;
  const simulationApi = commonJS ? require("./simulation.js") : window.AeonPhase5Simulation;
  ...
  function cloneAnchor(anchor) {
    return anchor.kind === "point"
      ? { kind: "point", destination: clonePoint(anchor.destination) }
      : { kind: "entity", entityId: anchor.entityId };
  }
  function cloneCommand(command) {
    ...
    if (command.anchor) result.anchor = cloneAnchor(command.anchor);
  ...
  function validateCommand(command, acceptedTick, expectedSequence) {
    ...
    if (["MOVE", "ATTACK_ENTITY", "ATTACK_MOVE", "STOP", "DEFEND"].includes(command.kind)) {
      ...
      if ((command.kind === "MOVE" || command.kind === "ATTACK_MOVE")
        && (!exactKeys(command.destination, ["x", "y"]) || !safeInteger(command.destination.x) || !safeInteger(command.destination.y))) {
        throw new Error("replay destination is invalid");
      }
      if (command.kind === "ATTACK_ENTITY" && !validIdentifier(command.targetId, configuration.structureIdMaxLength)) {
        throw new Error("replay target identifier is invalid");
      }
      if (command.kind === "DEFEND") {
        if (!plainObject(command.anchor) || (command.anchor.kind !== "point" && command.anchor.kind !== "entity")) throw new Error("replay defend anchor is invalid");
        if (command.anchor.kind === "point") {
          if (!exactKeys(command.anchor, ["destination", "kind"])
            || !exactKeys(command.anchor.destination, ["x", "y"])
            || !safeInteger(command.anchor.destination.x) || !safeInteger(command.anchor.destination.y)) throw new Error("replay defend anchor is invalid");
        } else if (!exactKeys(command.anchor, ["entityId", "kind"])
          || !validIdentifier(command.anchor.entityId, configuration.structureIdMaxLength)) throw new Error("replay defend anchor is invalid");
      }
    } else {
  ...
  function resolveOptions(options, label, allowUntilTick = false) {
    ...
    if (!map) throw new Error("Phase 5 replay requires the approved map");
  ...
  function executeReplay(replay, map, untilTick) {
    ...
    while (true) {
      ...
      const previousTick = simulation.tick;
      simulation.step();
      if (simulation.tick === previousTick && simulation.snapshot().match.status === "complete") break;
    }
  ...
  else window.AeonPhase5Replay = api;
}());
