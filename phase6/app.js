...
(function startPhase6Skirmish() {
  ...
  if (!map || !cameraApi || !baseRendererApi || !configApi || !skirmishApi
  ...
  function updateEconomy() {
    ...
    if (currentSnapshot.match?.status === "complete") {
      objectiveValue.textContent = currentSnapshot.match.winnerSeat === null
        ...
        : `${seatLabel(currentSnapshot.match.winnerSeat)} wins at tick ${currentSnapshot.match.completedTick}`;
    } else {
  ...
  function animationLoop(timestamp) {
    animationFrame = 0;
    if (destroyed || stage !== "battlefield" || !skirmish || !renderer) return;
    if (!effectivePaused()) {
  ...
  function syncInputState() {
    const inactive = stage !== "battlefield" || effectivePaused()
      || currentSnapshot?.match?.status === "complete";
    skirmish?.setSuspended(inactive);
    if (input) input.setEnabled(!inactive);
