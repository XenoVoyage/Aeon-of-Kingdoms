...
[![Status build v2026.8.22e](https://img.shields.io/badge/status_build-v2026.8.22e-6ef3ff)](CHANGELOG.md)
...
## Phase 4 approved · Phase 5 combat candidate in review

[Play the Phase 5 implementation candidate](phase5/) · [Read its frozen contract and evidence](docs/PHASE5_COMBAT_TACTICS.md) · [Play the approved Phase 4 foundation](phase4/)
...
The owner approved the complete Phase 3 entity/movement foundation on 2026-08-22 and authorized autonomous completion and publication of later verified phases. The subsequently approved Phase 4 foundation extends the same fixed 20 Hz simulation with exactly five instances of the three locked structure categories, deterministic capture, one Resource, population, five-item production queues, full refunds, ordered spawn validation, rally commands, and canonical replay/snapshot/checksum coverage. The local Phase 5 implementation candidate now adds the frozen focus, attack-move, defend, stop, acquisition, reservation, projectile, simultaneous-damage, defeat, structure-destruction, and headquarters-outcome rules plus twelve damage-state WebPs. It passes 57/57 focused checks and the complete dependency-free suite passes 225/225, but protected publication, deployed-byte verification, rendered review, and Phase 5 closure remain pending; Phase 6 is not yet authorized. Strategic AI, rooms, signaling, and networking transport remain absent. The selected future multiplayer direction remains a private two-player host/client room joined by a short code; GitHub Pages is only the static client host. The image above remains Phase 1A review evidence rather than a runtime screenshot. [`docs/STATUS.md`](docs/STATUS.md) owns exact source, deployment, evidence, and next-boundary state. The rejected prototype remains available at commit [`7f88655`](https://github.com/XenoVoyage/Aeon-of-Kingdoms/commit/7f88655f10504f44496fbba2e17871b16a5fe115) in Git history; its retired `v2026.8.15` tag and GitHub Release must not be recreated.

## Replacement target

The first replacement release remains one original layered landscape battlefield, two factions, faction-specific headquarters, shared Resource Points and Production Outposts, production and rally commands, readable formation combat, and strategic AI. The local simulation is proved before multiplayer. The [approved Phase 1B review](concepts/phase1b/) defines the visual and interaction target; the [closed Phase 2 foundation](docs/PHASE2_FOUNDATION.md) owns the landscape/camera base; the [closed Phase 3 contract](docs/PHASE3_ENTITY_MOVEMENT.md) owns entity movement; the [closed Phase 4 contract](docs/PHASE4_STRUCTURES_ECONOMY.md) owns structures and economy; and the [frozen Phase 5 contract](docs/PHASE5_COMBAT_TACTICS.md) owns the active implementation candidate and its still-pending gate.

## Preview the transition page locally

Open `index.html` in a modern browser for project status, `phase5/index.html` for the combat implementation candidate, `phase4/index.html` for the approved structures/economy foundation, `phase3/index.html` for the approved movement foundation, `phase2/index.html` for the approved landscape foundation, or `concepts/phase1b/index.html` for the approved script-free target. Node.js 20 or newer is used for the dependency-free source verification suite:
...
- [Frozen Phase 5 combat contract and implementation-candidate evidence](docs/PHASE5_COMBAT_TACTICS.md)
