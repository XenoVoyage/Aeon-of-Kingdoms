...
The published sample gate requires a named browser/version and viewport to load and decode all four exact review files, report their natural dimensions, report observed load/decode completion and available resource timing, and retain the exact decoded RGBA arithmetic as the bounded pixel-memory evidence. Browser/GPU bookkeeping and resource-release behavior remain separately unmeasured until an implemented loader exists; arithmetic is not mislabeled as a browser heap measurement.

## Evidence and approval boundary

Before protected publication, the source candidate must pass the focused Phase 1B source test, full dependency-free suite, exact Pages allowlist, diff hygiene, and direct-file review. After protected merge, the live Pages route, version, local resources, page-origin console, representative atlas decode, and responsive desktop/tablet/phone/portrait views must be checked before the candidate is handed to the owner. A failed live check requires a corrective protected pull request; it is not waived by a successful deployment.
