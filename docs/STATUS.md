...
| Area | Status |
| --- | --- |
| Canonical source version | `v2026.8.21b` in `VERSION.txt`; Phase 1B visual-lock review candidate selected for protected publication; intentionally untagged, with no GitHub Release planned |
...
| Active phase | Phase 1A is approved and closed; the complete non-playable Phase 1B visual-lock candidate is prepared for explicit owner review; Phase 2 remains blocked |
| Redesign gameplay implementation | Not started |
| Current source boundary | Minimal redesign status page, mood-reference archive, published approved Phase 1A Pages review, approved repository-only authoring masters, production-art contract, archived v5 source record, and one script-free Phase 1B visual-lock review candidate; none contains a playable redesign |
| GitHub Pages | `v2026.8.21a` remains the last verified deployed source from protected pull requests `#11` and `#12`; `v2026.8.21b` publication evidence is pending the current protected candidate workflow |
...
| Verification | Historical Phase 1A and publication evidence remains recorded below. The Phase 1B candidate adds a focused contract suite, four responsive review compositions, exact runtime-envelope measurements, and four bounded browser-decode sample files while preserving the 37-file approved Phase 1A subset and repository-only raw masters. Protected merge/Pages runs and live rendered checks remain pending the current workflow. No source or rendered check establishes physical-device behavior, gameplay, or owner approval |
...
| Deferred or unavailable | Physical-device landscape and broad browser-matrix observations; generalized stale-cache and rollback observations beyond the corrected stylesheet case; verify protection/ruleset settings that the GitHub connection could not expose; implement and browser-profile the selected Phase 1B export/load pipeline and final layered-map/damage-state assets only after the owner gate permits Phase 2 |
...
The published menu, battlefield, faction, structure, combat, mobile-control, and production/rally frames remain available in the [mood-reference gallery](../concepts/). The [published Phase 1A review](../concepts/feasibility/) presents the approved production direction reflected in the GDD: all six representatives and their actual-scale state playback, ownership, environment, damage, atlas audit, and desktop/phone compositions. The exact method is defined in [`PRODUCTION_ART.md`](PRODUCTION_ART.md), and repository-only authoring masters remain under `concepts/feasibility/phase1a/`. Its 37 staged visual assets are communication and asset-review evidence, not gameplay screenshots or implementation evidence. The [Phase 1B review candidate](../concepts/phase1b/) and [`PHASE1B_VISUAL_LOCK.md`](PHASE1B_VISUAL_LOCK.md) assemble the menu/HUD, complete twelve-identity language, battlefield, landscape controls, viewport matrix, and measured local asset-envelope proposal without adding executable gameplay. Source, staging, Actions, HTTP, and rendered-browser evidence can establish bounded publication; they do not establish physical-device readability, interaction, game feel, runtime quality, or owner approval. No redesigned map, entity, structure, movement, combat, AI, touch, or networking runtime exists yet.
...
### v2026.8.21b Phase 1B candidate evidence
...
The runtime-envelope audit cropped and resized every unique frame independently before packing, re-locked movement upper bodies, clamped masks to base alpha, and lossless-WebP round-tripped the result with ImageMagick 6.9.12-98 Q16 and libwebp 1.3.2. Six corrected 96-pixel base/mask pairs total 431,220 bytes; six corrected 128-pixel pairs total 688,988 bytes. Doubling complete representative sets projects 862,440 and 1,377,976 bytes for twelve identities, while exact decoded RGBA arithmetic is 13.5 MiB or 24 MiB when only one tier is resident. The corrected samples have zero upper-body differences, mask escapes, and lossless round-trip differences. Four exact Astral Guardian files totaling 183,574 bytes are staged as bounded browser-decode evidence for both tiers; they are not a complete roster or shipping runtime set.

The selected source passes the focused Phase 1B suite at 5/5, the complete dependency-free suite at 77/77, `git diff --check`, and exact staging at 64 allowlisted files plus `.nojekyll`. Protected pull-request checks, Pages publication, responsive rendered observations, and live browser sample-decode evidence remain pending the publication workflow. Physical phone/tablet evidence, broad browser/device evidence, implemented loading/fallback behavior, browser/GPU residency and release behavior, layered runtime-map budgets, complete structure damage-state bytes, gameplay, and owner approval remain unobserved.
...
1. Review the complete non-playable Phase 1B candidate and obtain explicit owner approval of its menu/HUD, battlefield, twelve identities, landscape controls, viewport matrix, and runtime-art envelope.
...
3. Preserve the published approved Phase 1A review and repository-only master boundary; keep the four Phase 1B atlas files explicitly bounded as review/browser-decode samples rather than treating them as a shipping runtime set.
