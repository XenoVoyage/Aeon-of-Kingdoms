...
(function startPhase4Candidate() {
  const map = window.AeonPhase4Map;
  const cameraApi = window.AeonPhase2Camera;
  const baseRendererApi = window.AeonPhase2Renderer;
  const configApi = window.AeonPhase4Config;
  const simulationApi = window.AeonPhase4Simulation;
  const replayApi = window.AeonPhase4Replay;
  const entityAssetsApi = window.AeonPhase3Assets;
  const structureAssetsApi = window.AeonPhase4Assets;
  const dynamicRendererApi = window.AeonPhase4Renderer;
  const inputApi = window.AeonPhase4Input;
  ...
  const mapStructures = new Map(map.phase4.structures.map((structure) => [structure.id, structure]));
  ...
  function updateEconomy() {
    ...
    const shared = currentSnapshot.structures.filter((structure) => structure.category !== "headquarters");
    const owned = shared.filter((structure) => structure.ownerSeat === 1 && !structure.destroyed).length;
    objectiveValue.textContent = owned === shared.length
      ? `All ${shared.length} shared structures controlled`
      : `${owned} / ${shared.length} shared structures controlled`;
  }

  function setContextMode(mode) {
    const activeMode = input ? input.setContextMode(mode) : null;
    moveModeButton.setAttribute("aria-pressed", String(activeMode === "move"));
    rallyModeButton.setAttribute("aria-pressed", String(activeMode === "rally"));
    moveModeButton.textContent = activeMode === "move" ? "Choose terrain" : "Move";
    rallyModeButton.textContent = activeMode === "rally" ? "Choose terrain" : "Rally";
  }
  ...
  function updateSelectionUi() {
    ...
    if (structure) {
      ...
      const captureLabel = presented.contested
        ...
        : capture?.challengerSeat
          ...
          : "";
      selectionDetail.textContent = `${ownerLabel(structure.ownerSeat)}${structure.destroyed ? " · Destroyed" : captureLabel}`;
    } else if (entities.length === 1) {
      selectionSummary.textContent = titleWords(entities[0].kind);
      selectionDetail.textContent = `${ownerLabel(entities[0].ownerSeat)} · ${entities[0].order === "MOVE" ? "Moving" : "Ready"}`;
    } else if (entities.length > 1) {
      selectionSummary.textContent = `${entities.length} Astral entities`;
      selectionDetail.textContent = "Formation selection · Player 1 ◇";
    } else {
      selectionSummary.textContent = "None";
      selectionDetail.textContent = "Choose combat entities or a structure.";
    }
    ...
    moveModeButton.disabled = entities.length === 0 || effectivePaused();
    rallyModeButton.disabled = !producer || effectivePaused();
    clearRallyButton.disabled = !producer || structure.rally === null || effectivePaused();
  ...
  function hitAtWorldPoint(point) {
  ...
  function onContextRequest(payload) {
    ...
    submit("MOVE", { entityIds: Array.from(selectedEntityIds).sort(compareIdentifiers), destination }, destination);
  }
      ...
      if (event.type === "command") {
        ...
        if (event.status === "rejected") {
          ...
          destinationFeedback.splice(0, destinationFeedback.length, {
            ...
            status: "accepted",
            label: pending.kind === "SET_RALLY" ? "Rally set" : "Moving"
          });
          announce(pending.kind === "SET_RALLY" ? "Rally point set" : "Command applied");
        } else {
        ...
        if (selectedEntityIds.has(event.entityId)) {
          ...
          else if (event.status === "stopped") announce("An entity stopped after bounded congestion recovery");
        }
  ...
  function createRuntime(loadedGround, loadedEntityAssets, loadedStructureAssets) {
    ...
    input = inputApi.createInput({
      ...
      onContextModeChange(mode) {
        moveModeButton.setAttribute("aria-pressed", String(mode === "move"));
        rallyModeButton.setAttribute("aria-pressed", String(mode === "rally"));
        moveModeButton.textContent = mode === "move" ? "Choose terrain" : "Move";
        rallyModeButton.textContent = mode === "rally" ? "Choose terrain" : "Rally";
      },
      onTransientReset() { selectionMarquee.hidden = true; }
    });
  ...
  async function startBattlefield(options) {
    ...
    stage = "battlefield";
    shell.dataset.stage = stage;
    notice = "Loading approved local terrain, entity art, and structure art";
    updateViewport();
  ...
  function onMoveModeToggle() { setContextMode(input?.snapshot().contextMode === "move" ? null : "move"); }
  function onRallyModeToggle() { setContextMode(input?.snapshot().contextMode === "rally" ? null : "rally"); }
