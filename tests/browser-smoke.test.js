...
test("public shell truthfully presents the phased redesign", () => {
  ...
  assert.match(currentPhaseItems[0], /Authorized · implementation next/i);
  ...
  assert.match(html, /href=["']docs\/PHASE3_ENTITY_MOVEMENT\.md["']/i);
...
test("service-worker upgrade bypasses stale HTTP cache and refreshes only the retired entry page", async () => {
  ...
  const caches = {
    open(name) {
      assert.equal(name, "aok-shell-v2026.8.22");
