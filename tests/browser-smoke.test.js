...
test("public shell truthfully presents the phased redesign", () => {
  ...
  for (const pattern of [
    ...
    /id=["']main-content["']/i,
    /id=["']page-title["']/i,
    /Phase 3 complete · Phase 4 candidate/i,
    /Redesign in progress/i,
    /Phase 1A, Phase 1B, and the complete Phase 2 landscape\/camera foundation are approved/i,
    /Play the Phase 4 candidate/i,
    /<span class=["']phase-number["']>04<\/span>/i,
    /Phase 4 is ready for integrated review/i,
    /Status build v2026\.8\.22c/i
...
test("service worker replaces old game caches with only the status shell", () => {
  ...
  assert.match(worker, /\$\{CACHE_PREFIX\}v2026\.8\.22/);
...
test("service-worker upgrade bypasses stale HTTP cache and refreshes only the retired entry page", async () => {
  ...
  const caches = {
    open(name) {
      assert.equal(name, "aok-shell-v2026.8.22c");
...
test("Pages staging contains approved public routes and source-of-truth links but no rejected runtime", () => {
...
test("Pages-subpath delivery serves every linked local status resource", async (context) => {
  ...
  for (const route of ["concepts/", "concepts/feasibility/", "concepts/phase1b/", "phase2/", "phase3/", "phase4/"]) {
