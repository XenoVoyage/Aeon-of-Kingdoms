...
The active source boundary is a non-playable redesign status page, a reviewed mood-reference archive, a superseded v5 Phase 1A proof, an approved production-art contract, and a complete direct-file Phase 1A feature-branch candidate whose corrected Aegis Titan awaits owner confirmation. Verification proves that these boundaries and files are truthful, local, bounded, and internally consistent; it does not validate redesigned gameplay, approve an individual asset or the complete set, establish publication, or replace owner playback and physical-device review. The rejected `v2026.8.15` prototype and its old tests remain historical evidence only.
...
| `feasibility-gallery.test.js` | Integrity, delivery, accessibility, dimensions, payloads, and historical claims of the superseded v5 Phase 1A proof; it does not define or approve the current animation method |
...
3. Run `node .github/scripts/stage-pages.js _site` and verify the stage contains only `.nojekyll`, the four-file status shell, the four public status/contract documents, and the explicitly allowlisted mood and Phase 1A review surfaces: 32 generated files total (`.nojekyll` plus 31 allowlisted files).
...
| Superseded Phase 1A proof | When auditing historical delivery, inspect the eleven v5 assets and `concepts/feasibility/` without treating their six-frame timing, authored facing, animation quality, or owner-gate language as current production approval |
| Current production-art review | Follow `concepts/feasibility/phase1a/README.md` and `docs/PRODUCTION_ART.md`: inspect the four primary compositions, six canonical master atlases, masks, exact mirror output, actual desktop/phone-scale playback, roots, equipment, anatomy, cadence, structure ownership, damage, and color variants; record the complete owner decision separately |
